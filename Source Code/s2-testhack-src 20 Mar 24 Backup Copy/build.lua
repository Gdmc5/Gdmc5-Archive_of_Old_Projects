...
local tools, platform_directory = common.find_tools("s2p2bin", "fixpointer", "saxman", "convsym")
...
-- Delete old ROM.
os.remove("s2testhackbuilt.prev.bin")

-- Backup the most recent ROM.
os.rename("s2testhackbuilt.bin", "s2testhackbuilt.prev.bin")

-- Assemble the ROM.
local assemble_result = common.assemble_file("s2.asm", "s2testhackbuilt.bin", tools.as, "", tools.s2p2bin, improved_sound_driver_compression and "" or "-a", true)
...
os.execute(tools.fixpointer .. " s2.h s2testhackbuilt.bin   off_3A294 MapRUnc_Sonic 0x2D 0 4   word_728C_user Obj5F_MapUnc_7240 2 2 1")

-- Remove the header file, since we no longer need it.
os.remove("s2.h")

-- Correct the ROM's header with a proper checksum and end-of-ROM value.
common.fix_header("s2testhackbuilt.bin")
 
-- Run convsym to add the symbol table to the end of the ROM.
os.execute(tools.convsym .. " s2.lst s2testhackbuilt.bin -input as_lst -a")
